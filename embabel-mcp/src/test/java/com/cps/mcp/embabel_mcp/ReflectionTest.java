...
public class ReflectionTest {
                        ...
                        if (args != null && args.length > 0) {
                            if ("addObject".equals(methodName)) {
                                ...
                                if (isFilteredUserInput(arg)) {
                                    System.out.println(
                                            ">>> Blackboard Proxy: Filtering out addObject for UserInput: " + arg);
                                ...
                                if (args.length >= 2) {
                                    ...
                                    if (isFilteredUserInput(value)) {
                                        if (args[0] != null && "it".equals(args[0].toString())) {
                                            System.out.println(
                                                    ">>> Blackboard Proxy: Allowing bind/bindProtected for key \"it\" -> "
                                                            + value);
                                        } else {
                                            System.out.println(
                                                    ">>> Blackboard Proxy: Filtering out bind/bindProtected for: "
                                                            + args[0] + " -> " + value);
                                            if (method.getReturnType().equals(Blackboard.class)) {
                                                return proxy;
                                            }
                                    ...
                                    for (java.util.Map.Entry<?, ?> entry : originalMap.entrySet()) {
                                        if (isFilteredUserInput(entry.getValue())) {
                                            if (entry.getKey() != null && "it".equals(entry.getKey().toString())) {
                                                System.out.println(
                                                        ">>> Blackboard Proxy: Allowing bindAll entry for key \"it\" -> "
                                                                + entry.getValue());
                                                filteredMap.put(entry.getKey(), entry.getValue());
                                            } else {
                                                System.out.println(">>> Blackboard Proxy: Filtering out bindAll entry: "
                                ...
                                if (args.length >= 2 && args[0] != null) {
                                    ...
                                    if ("userInput".equals(key) || "input".equals(key)) {
                                        return new com.embabel.agent.domain.io.UserInput(goalStr);
                                    } else if ("it".equals(key)) {
                                        if (args[1] != null && args[1].toString().contains("UserInput")) {
                                            return new com.embabel.agent.domain.io.UserInput(mappedGoalStr);
                                        }
                                    }
                                }
                                ...
                                if (args.length >= 1 && args[0] != null && args[0].toString().contains("UserInput")) {
                                    return List.of(new com.embabel.agent.domain.io.UserInput(goalStr));
                                }
                            }
                            ...
                            if ("getObjects".equals(methodName)) {
                                ...
                                modifiedObjects.add(new com.embabel.agent.domain.io.UserInput(goalStr));
                                modifiedObjects.add(new com.embabel.agent.domain.io.UserInput(mappedGoalStr));
    ...
    public void verifyBlackboardInjectionBehavior() throws Exception {
        java.io.File logFile = new java.io.File(
                "C:\\Users\\atmar\\.gemini\\antigravity-ide\\brain\\6182c3cd-c089-457d-a52c-20e20cc241ef\\test_run.log");
        ...
        try {
            ...
            Blackboard proxyBlackboard = createFilteringBlackboardProxy(childBlackboard, "Weather in Rome",
                    "Provide weather forecast");

            // 2. Add original UserInput to blackboard
            com.embabel.agent.domain.io.UserInput userInput = new com.embabel.agent.domain.io.UserInput(
                    "Weather in Rome");
